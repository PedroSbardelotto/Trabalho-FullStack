...
class EventoService {
  ...
  criarEvento(evento: Evento): Evento {
    if (!evento.nome || !evento.local || !evento.horario || evento.valor <= 0 || evento.quantidadeDisponivel <= 0) {
      throw new Error('Todos os campos do evento são obrigatórios e devem ser válidos');
    }
